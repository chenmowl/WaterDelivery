 ...
 */
public class AssessTicketRecordAdapter extends BaseQuickAdapter<AssessTicketRecordVo.ResultsBean, BaseViewHolder> {

    private LayoutInflater inflater;

    public AssessTicketRecordAdapter(Context context, List<AssessTicketRecordVo.ResultsBean> data) {
        super(R.layout.item_assess_ticket, data);
        inflater = LayoutInflater.from(context);
    }

    @Override
    protected void convert(BaseViewHolder helper, AssessTicketRecordVo.ResultsBean item) {
//        helper.setText(R.id.tv_receiver, "收件人:"+item.getName());
        ...
        List<AssessTicketRecordVo.ResultsBean.TiketsDayCountBean> tiketsDayCount = item.getTiketsDayCount();
        if (tiketsDayCount != null && tiketsDayCount.size() != 0) {
            LinearLayout ll_container = helper.getView(R.id.ll_container);
            ll_container.removeAllViews();
            for (AssessTicketRecordVo.ResultsBean.TiketsDayCountBean tiketsDayCountBean : tiketsDayCount) {
