...
import com.eme.waterdelivery.presenter.ApplyTicketRecordPresenter;
import com.eme.waterdelivery.tools.ToastUtil;
import com.eme.waterdelivery.ui.adapter.ApplyTicketRecordAdapter;
 ...
 */
public class ApplyTicketRecordActivity extends BaseActivity<ApplyTicketRecordPresenter> implements ApplyTicketRecordContract.View, SwipeRefreshLayout.OnRefreshListener, BaseQuickAdapter.RequestLoadMoreListener {
    ...
    protected void initEventAndData(Bundle savedInstanceState) {
        tvTitle.setText(getText(R.string.apply_ticket_record));
        ...
        mData = new ArrayList<>();
        ApplyTicketRecordAdapter adapter = new ApplyTicketRecordAdapter(this, mData);
    ...
    public void onRefresh() {
        ((ApplyTicketRecordAdapter)rvContent.getAdapter()).setEnableLoadMore(false);
    ...
    public void netError(int flag) {
        switch (flag) {
            ...
            case Constant.REFRESH_DOWN:
                ((ApplyTicketRecordAdapter)rvContent.getAdapter()).setEnableLoadMore(true);
                break;
            case Constant.REFRESH_UP_LOADMORE:
                swipeRefresh.setEnabled(true);
                ((ApplyTicketRecordAdapter)rvContent.getAdapter()).loadMoreFail();
                break;
            default:
                break;
        }
    ...
    public void notifyNoData() {
        ...
        swipeRefresh.setEnabled(true);
        ((ApplyTicketRecordAdapter) rvContent.getAdapter()).loadMoreEnd();
    }
    ...
    public void showRequestListResult(boolean isSuccess, List<SaleTicketRecordBo.ListBean> list, int refreshFlag, String message) {
        if(isSuccess){
            ...
            if(Constant.REFRESH_UP_LOADMORE==refreshFlag){
                ((ApplyTicketRecordAdapter)rvContent.getAdapter()).loadMoreComplete();
            }
        }else{
            switch (refreshFlag) {
                case Constant.REFRESH_NORMAL:
