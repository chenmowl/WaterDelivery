...
import com.eme.waterdelivery.presenter.AssessTicketRecordPresenter;
import com.eme.waterdelivery.tools.ToastUtil;
import com.eme.waterdelivery.ui.adapter.AssessTicketRecordAdapter;
 ...
 */
public class AssessTicketRecordFragment extends BaseFragment<AssessTicketRecordPresenter> implements AssessTicketRecordContract.View,SwipeRefreshLayout.OnRefreshListener, BaseQuickAdapter.RequestLoadMoreListener {
    ...
    private List<AssessTicketRecordVo.ResultsBean> listData;
    ...
    protected void initEventAndData() {
        ...
        AssessTicketRecordAdapter fixedAdapter = new AssessTicketRecordAdapter(mActivity,listData);
...
//        fixedAdapter.openLoadAnimation(BaseQuickAdapter.ALPHAIN);
    ...
    public void requestFailure(int flag) {
        switch (flag) {
            ...
            case Constant.REFRESH_DOWN:
                ...
                ((AssessTicketRecordAdapter)rvContent.getAdapter()).setEnableLoadMore(true);
                break;
            case Constant.REFRESH_UP_LOADMORE:
                ((AssessTicketRecordAdapter)rvContent.getAdapter()).loadMoreFail();
    ...
    @Override
    public void updateUi(List<AssessTicketRecordVo.ResultsBean> data, int flag) {
        switch (flag) {
            case Constant.REFRESH_NORMAL:
                listData.clear();
                isShowLayer(llAvLoadingTransparent44, false);
        ...
        if(Constant.REFRESH_UP_LOADMORE==flag){
            ((AssessTicketRecordAdapter)rvContent.getAdapter()).loadMoreComplete();
        }
    }
    ...
    public void notifyNoData() {
        ...
        swipeRefresh.setEnabled(true);
        ((AssessTicketRecordAdapter)rvContent.getAdapter()).loadMoreEnd();
    }
    ...
    public void onRefresh() {
        ((AssessTicketRecordAdapter)rvContent.getAdapter()).setEnableLoadMore(false);
